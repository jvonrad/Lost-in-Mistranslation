...
torchrun --standalone --nproc_per_node=4 training/train_ted_structured_full.py

python training/train_ted_structured_full.py --model_id olmo2-7b --learning_rate 5e-6 --num_epochs 1 --per_device_train_batch_size 1 --gradient_accumulation_steps 8
...
def default_ckpt_dir(model_id: str) -> str:
	model_name = model_id.split("/")[-1]
	return os.path.join(MODEL_OUT_ROOT, f"{model_name}-ted-structured-full-checkpoints")


def default_output_dir(model_id: str) -> str:
	model_name = model_id.split("/")[-1]
	return os.path.join(MODEL_OUT_ROOT, f"{model_name}-ted-structured-full-final")
...
DATASET_PATH = ""
REQ_LANGS = ["en", "es", "fr", "de", "id", "pt", "ru", "zh", "ja", "ar", "sw", "bn"]
...
def compute_flores_hidden_cosine(
    ...
    for lang, data in flores_sets.items():
        ...
        for i in range(0, len(src_texts), batch_size):
            ...
            all_mid.extend(F.cosine_similarity(src_mid, tgt_mid, dim=-1).detach().float().cpu().tolist())
            all_last.extend(F.cosine_similarity(src_last, tgt_last, dim=-1).detach().float().cpu().tolist())
...
def get_sentence_embeddings(
    ...
    for i in range(0, len(texts), batch_size):
        ...
        pooled = mean_pool_hidden(hidden, tok["attention_mask"])
...
def main():
	...
	if args.report_to == "wandb" and rank == 0:
		wandb.init(
            ...
            name=f"{args.model_id.split('/')[-1]}-full-lr{args.learning_rate}",
			config={
				"model": args.model_id,
	...
	# ── 4. Load full model for full fine-tuning ────────────────────────────────
	if "model" in globals():
		del globals()["model"]
		torch.cuda.empty_cache()

	if rank == 0:
		print(f"\nLoading {args.model_id} for FULL fine-tuning...")
	...
	if rank == 0:
		print(f"\nStarting OLMo-2 run:")
		print(f"  Format: grouped-paired | Full Parameter Fine-Tuning | no quantization")
	...
	print(f"\nDone. Full model saved to {args.output_dir}")
