...
# ---------------------------------------------------------------------------
# Qwen-2 head-mask patch
# ---------------------------------------------------------------------------
def patch_qwen2_for_head_mask(model):
    """
    Monkey-patch Qwen2Attention layers for LAHIS.
    Unlike OLMo-2, Qwen-2 does NOT use q_norm or k_norm.
    """
    try:
        ...
        raise ImportError(
            "transformers Qwen2 utilities not found. "
            "Install or update transformers."
        ) from e
        ...
        def _make_forward(attn_module, l_idx):
            def _forward(
                ...
                **kwargs,
                ...
                # Handle varying kwarg names across transformer versions
                ...
                attn_weights = F.dropout(attn_weights, p=attn_dropout, training=attn_module.training)

                attn_output = torch.matmul(attn_weights, value_states)

                # --- LAHIS head mask application ---
                if attn_module._lahis_mask is not None:
                    mask = attn_module._lahis_mask.to(
                        device=attn_output.device, dtype=attn_output.dtype
                    )
...
def load_model(
    ...
    if model_name.startswith("hf:"):
        ...
        patch_type = "hf"          # assume OLMo-2 architecture for finetuned models
    elif local:
        model_dict = {
            "aya":    AYA_NAME_LOCAL,
...
# ── Apply LAHIS patch ─────────────────────────────────────────────────────
    if patch_type == "llama2":
        ...
        print("  Patch applied.")
    elif patch_type == "olmo2":
        print("Applying LAHIS head-mask patch to OLMo-2 attention layers...")
        patch_olmo2_for_head_mask(model)
        print("  Patch applied.")
    elif patch_type == "hf":
        # Dynamically detect architecture for custom HF paths
        model_type = getattr(model.config, "model_type", "").lower()
        if "qwen2" in model_type:
            print("Applying LAHIS head-mask patch to Qwen-2 attention layers...")
            patch_qwen2_for_head_mask(model)
            print("  Patch applied.")
            ...
            print(f"Fallback: Applying Llama patch for architecture '{model_type}'...")
            patch_llama_for_head_mask(model)
            print("  Patch applied.")
